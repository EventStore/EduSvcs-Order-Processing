...
class Subscriber implements AsyncDisposable {
    ...
    private readonly encoder: EventEncoder<OrderCommand>;
    ...
    constructor(
        ...
        this.handler = new OrderHandler(clock);
        this.encoder = new JsonEventEncoder<OrderCommand>();
                ...
                if (event.event && event.event.isJson) {
                    ...
                    switch (command._tag) {
                        case "Some":
                            ...
                            const envelope: Envelope<OrderCommand> = {
...
export class OrderPersistentSubscriber {
    private readonly name: string;
    ...
    constructor(name: string, clock: Clock) {
        this.name = name;
        this.clock = clock;
        this.event_filter = new MessageFilter([ "sent.cancel-order", "sent.confirm-order" ]);
    }
